...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: DNDDrop1.c /main/10 1996/10/07 14:54:59 drk $"
#endif
#endif
 ...
 *
 *	This test also shows the bug about non-Pixel unitType
 *
 */
...
Widget  Whichwidget_Pulldown;
Widget  Whichwidget_Option;
Widget  PB0, PB1, PB2, All;
									 ...
									 XtPointer call_data)
{
	...
	char *which_widget = (char *)client_data;
	...
	if (strcmp (which_widget, "PushBtn0") == 0)
	  XmDropSiteUpdate(dropSites[0], args, n);

	else if (strcmp (which_widget, "PushBtn2") == 0)
	  XmDropSiteUpdate(dropSites[1], args, n);

	else if (strcmp (which_widget, "PushBtn4") == 0)
	  XmDropSiteUpdate(dropSites[2], args, n);

	else if (strcmp (which_widget, "All") == 0)
...

{
	...
	XtAddCallback(dropIgnore, XmNactivateCallback, dropIgnoreCB, NULL);
	...
	Whichwidget_Pulldown = XmCreatePulldownMenu(Form, "Whichwidget_Pulldown",
						args, n);
	PB0 = CreatePushButton(Whichwidget_Pulldown, "PushBtn0");
	PB1 = CreatePushButton(Whichwidget_Pulldown, "PushBtn2");
	PB2 = CreatePushButton(Whichwidget_Pulldown, "PushBtn4");
	All = CreatePushButton(Whichwidget_Pulldown, "All");



	XtAddCallback(PB0, XmNactivateCallback, applyCB, 
		      (XtPointer)"PushBtn0");
	XtAddCallback(PB1, XmNactivateCallback, applyCB, 
		      (XtPointer)"PushBtn2");
	XtAddCallback(PB2, XmNactivateCallback, applyCB, 
		      (XtPointer)"PushBtn4");
	XtAddCallback(All, XmNactivateCallback, applyCB, 
		      (XtPointer)"All");


    tcs = XmStringLtoRCreate("Apply To         ",
			      XmSTRING_DEFAULT_CHARSET);

	n = 0;
	XtSetArg(args[n], XmNlabelString, tcs); n++;
	XtSetArg(args[n], XmNmenuHistory, All); n++;
	XtSetArg(args[n], XmNsubMenuId, Whichwidget_Pulldown); n++;
	XtSetArg(args[n], XmNbackground, CommonGetColor("darkgreen")); n++;
	XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNtopWidget, WhichdragOp_Option); n++;
	XtSetArg(args[n], XmNtopOffset, 5); n++;
	...
	XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNrightOffset, 5); n++;
	Whichwidget_Option = XmCreateOptionMenu(Form, 
						"Whichwidget_Option", 
						args, n);
	XtManageChild(Whichwidget_Option);
	...
	for (i = 0; i < appData->numShells; i++) {
		...
		for (j = 0; j < appData->numCols; j++)
			for (k = 0; k < appData->numRows; k++) {
				...
				n = 0;
				XtSetArg(args[n], XtNx, 10 + j * 75); n++;
				XtSetArg(args[n], XtNy, 10 + k * 75); n++;
	...
	if (use_instruction_box == True) {
		...
		XtSetValues (dropSites[0], args, n); 
	...
	while (*user_data != '\0') {
		...
		if (strcmp(next_word, "-noinstruct") == 0) {
			...
			continue;
											...
											/* Begin CR4924 */
		if (strcmp(next_word, "showunitbug") == 0) {
			show_unit_bug = True;
			continue;
